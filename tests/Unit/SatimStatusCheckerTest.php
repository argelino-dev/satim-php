...
it('returns success message when transaction is successful', function () {
    $this->helper->setResponse(['OrderStatus' => '2', 'params' => ['respCode_desc' => 'Success']]);

    expect($this->helper->getSuccessMessage())->toBe('Success');
});

it('returns default success message if success response is missing', function () {
...
it('returns rejection message when transaction is rejected', function () {
...
it('returns refund message when transaction is refunded', function () {
    $this->helper->setResponse(['OrderStatus' => '4']);

    expect($this->helper->getErrorMessage())->toBe('Payment was refunded');
});

it('returns default error message if error response is missing', function () {
...
// ✅ Test: isSuccessful()
it('returns true if transaction is successful', function () {
    $this->helper->setResponse(['OrderStatus' => '2']);

    expect($this->helper->isSuccessful())->toBeTrue();
});

it('returns false if transaction is not successful', function () {
    $this->helper->setResponse(['OrderStatus' => '3']);
...
// ✅ Test: isFailed()
it('returns true if transaction failed', function () {
    $this->helper->setResponse(['OrderStatus' => '3']);

    expect($this->helper->isFailed())->toBeTrue();
});

it('returns false if transaction did not fail', function () {
    $this->helper->setResponse(['OrderStatus' => '2']);
...
// ✅ Test: isRefunded()
it('returns true if transaction is refunded', function () {
    $this->helper->setResponse(['OrderStatus' => '4']);

    expect($this->helper->isRefunded())->toBeTrue();
});

it('returns false if transaction is not refunded', function () {
    $this->helper->setResponse(['OrderStatus' => '2']);
...
// ✅ Test: isCancelled()
it('returns true if transaction is cancelled', function () {
    $this->helper->setResponse(['actionCode' => '10']);

    expect($this->helper->isCancelled())->toBeTrue();
});

it('returns false if transaction is not cancelled', function () {
    $this->helper->setResponse(['actionCode' => '0']);

    expect($this->helper->isCancelled())->toBeFalse();
});

it('returns true if cancellation message is in ErrorMessage', function () {
...
// ✅ Test: isExpired()
it('returns true if transaction expired', function () {
    $this->helper->setResponse(['actionCode' => '-2007']);

    expect($this->helper->isExpired())->toBeTrue();
});

it('returns false if transaction is not expired', function () {
    $this->helper->setResponse(['actionCode' => '0']);
...
it('throws exception when getSuccessMessage() is called without response data', function () {
    $this->helper->getSuccessMessage();
})->throws(SatimMissingDataException::class, 'No response data found.');

it('throws exception when getErrorMessage() is called without response data', function () {
    $this->helper->getErrorMessage();
})->throws(SatimMissingDataException::class, 'No response data found.');

it('throws exception when isRejected() is called without response data', function () {
    $this->helper->isRejected();
})->throws(SatimMissingDataException::class, 'No response data found.');

it('throws exception when isSuccessful() is called without response data', function () {
    $this->helper->isSuccessful();
})->throws(SatimMissingDataException::class, 'No response data found.');

it('throws exception when isFailed() is called without response data', function () {
    $this->helper->isFailed();
})->throws(SatimMissingDataException::class, 'No response data found.');

it('throws exception when isRefunded() is called without response data', function () {
    $this->helper->isRefunded();
})->throws(SatimMissingDataException::class, 'No response data found.');

it('throws exception when isCancelled() is called without response data', function () {
    $this->helper->isCancelled();
})->throws(SatimMissingDataException::class, 'No response data found.');

it('throws exception when isExpired() is called without response data', function () {
...
it('isCancelled returns false if errorCode is 0', function () {
    $this->helper->setResponse(['errorCode' => '0']);

    expect($this->helper->isCancelled())->toBeFalse();
});

it('returns false if errorCode is 0', function () {
    $this->helper->setResponse(['errorCode' => '0']);

    expect($this->helper->isRejected())->toBeFalse();
});

it('returns true if actionCode is 2003', function () {
